...
router.get("/redirect", async (req: Request, res: Response) => {
    ...
    fetch(canvasUrl + "/login/oauth2/token", {
        ...
        console.log("selfEnrollments:", selfEnrollments); // Log enrollments for debugging
        ...
        // Process each course for both instructors and students
        for (const course of courses) {
            if (role === "Instructor") {
                // Handle Instructor course creation/updating
            ...
            // Now process enrollments
            const dbCourse = await prisma.course.findUnique({
                where: { canvasId: course.id.toString() },
            });

            if (!dbCourse) {
                console.log("Course not found in DB, skipping:", course.id); // Log for debugging
                continue;  // Skip if course doesn't exist in DB
            }

            console.log("dbCourse found:", dbCourse); // Log dbCourse data for debugging

            // Loop through each enrollment to create an Enrollment for the user
            for (const enrollment of selfEnrollments) {
                if (enrollment.type === "StudentEnrollment" && enrollment.course_id === course.id) {
                    console.log("Creating/updating enrollment:", enrollment); // Log each enrollment being processed
